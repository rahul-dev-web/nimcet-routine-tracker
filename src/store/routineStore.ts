...
export const useRoutineStore = create<RoutineState>((set, get) => ({
  ...
  setProfile: (profile) => {
    const safeProfile: UserProfile = { ...profile, studyTargetHours: Math.min(16, Math.max(1, Number(profile.studyTargetHours) || DEFAULT_PROFILE.studyTargetHours)) };
    set({ profile: safeProfile }); get().saveToLocalStorage();
  },

  setCollegePlan: (date, goingToCollege) => {
    set((state) => {
      const dailyPlans = new Map(state.dailyPlans);
      dailyPlans.set(date, { date, college: goingToCollege ? "going" : "not_going", answeredAt: new Date().toISOString(), autoDefaulted: false });
      return { dailyPlans };
  ...
  getCurrentTask: (date) => {
    const state = get(); const targetDate = date ?? getTodayDateKey(); const routine = state.getRoutineForDate(targetDate);
    const currentMinutes = getLocalTimeMinutesFromString(state.currentTime); const progress = state.getDailyProgress(targetDate);
    return routine.find((task) => !getTaskCompleted(progress, task.id) && isTaskCurrent(task, currentMinutes)) ?? null;
  },

  getNextTask: (date) => {
    const targetDate = date ?? getTodayDateKey(); const routine = get().getRoutineForDate(targetDate); const progress = get().getDailyProgress(targetDate); const current = get().getCurrentTask(targetDate);
  ...
  loadFromLocalStorage: () => {
    ...
    if (stored) {
      try {
        const parsed = JSON.parse(stored); const version = parsed.routineSchemaVersion ?? 1;
        set({ profile: { ...DEFAULT_PROFILE, ...(parsed.profile || {}) }, routineSchemaVersion: ROUTINE_SCHEMA_VERSION, dailyPlans: new Map(parsed.dailyPlans || []), dailyProgress: new Map(parsed.dailyProgress || []) });
        if (version < ROUTINE_SCHEMA_VERSION) get().saveToLocalStorage();
      } catch { console.error("Failed to load from localStorage"); }
    }
  ...
  saveToLocalStorage: () => {
    if (typeof window === "undefined") return;
    const state = get();
    localStorage.setItem("routineStore", JSON.stringify({ routineSchemaVersion: ROUTINE_SCHEMA_VERSION, profile: state.profile, dailyPlans: Array.from(state.dailyPlans.entries()), dailyProgress: Array.from(state.dailyProgress.entries()) }));
  },
}));

function getLocalTimeMinutesFromString(time: string): number { const [h, m] = time.split(":").map(Number); return h * 60 + m; }
